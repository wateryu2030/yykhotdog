...
export const REDIS_CONFIG = {
  ...
  main: {
    nodes: [
      {
        host: process.env['REDIS_MAIN_HOST_1'] || 'localhost',
        port: parseInt(process.env['REDIS_MAIN_PORT_1'] || '6379'),
      },
      {
        host: process.env['REDIS_MAIN_HOST_2'] || 'localhost',
        port: parseInt(process.env['REDIS_MAIN_PORT_2'] || '6380'),
      },
      {
        host: process.env['REDIS_MAIN_HOST_3'] || 'localhost',
        port: parseInt(process.env['REDIS_MAIN_PORT_3'] || '6381'),
      },
    ],
    options: {
      redisOptions: {
  ...
  session: {
    nodes: [
      {
        host: process.env['REDIS_SESSION_HOST_1'] || 'localhost',
        port: parseInt(process.env['REDIS_SESSION_PORT_1'] || '6382'),
      },
      {
        host: process.env['REDIS_SESSION_HOST_2'] || 'localhost',
        port: parseInt(process.env['REDIS_SESSION_PORT_2'] || '6383'),
      },
    ],
    options: {
      redisOptions: {
  ...
  cache: {
    nodes: [
      {
        host: process.env['REDIS_CACHE_HOST_1'] || 'localhost',
        port: parseInt(process.env['REDIS_CACHE_PORT_1'] || '6384'),
      },
      {
        host: process.env['REDIS_CACHE_HOST_2'] || 'localhost',
        port: parseInt(process.env['REDIS_CACHE_PORT_2'] || '6385'),
      },
      {
        host: process.env['REDIS_CACHE_HOST_3'] || 'localhost',
        port: parseInt(process.env['REDIS_CACHE_PORT_3'] || '6386'),
      },
    ],
    options: {
      redisOptions: {
...
export function getMainCluster(): Cluster {
  if (!mainCluster) {
    mainCluster = new Cluster(REDIS_CONFIG.main.nodes, REDIS_CONFIG.main.options);
...
export function getSessionCluster(): Cluster {
  if (!sessionCluster) {
    sessionCluster = new Cluster(REDIS_CONFIG.session.nodes, REDIS_CONFIG.session.options);
...
export function getCacheCluster(): Cluster {
  if (!cacheCluster) {
    cacheCluster = new Cluster(REDIS_CONFIG.cache.nodes, REDIS_CONFIG.cache.options);
...
export class UserCacheService {
  ...
  static async clearUserCache(userId: number): Promise<void> {
    const keys = [CacheKeyGenerator.userSession(userId), CacheKeyGenerator.userPermissions(userId)];
