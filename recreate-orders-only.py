...
def recreate_orders():
    ...
    try:
        ...
        logger.info(f"从cyrg2025获取到 {len(orders_data)} 个订单")
        ...
        conn.commit()
        logger.info(f"成功插入 {len(orders_data)} 个订单")
...
def main():
    """主函数"""
    logger.info("开始重新创建订单数据...")
    
    # 重新创建订单数据
    if not recreate_orders():
