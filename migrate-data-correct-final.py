#!/usr/bin/env python3
"""
最终正确版数据迁移脚本 - 从cyrg2025和cyrgweixin迁移到hotdog2030
使用正确的源数据库列名
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('migration_correct_final.log'),
...
def migrate_products(conn):
    ...
    try:
        ...
        SELECT 
            ...
            g.goodsName,
            g.categoryId,
            ISNULL(g.salePrice, 0) as price,  -- 使用salePrice而不是goodsPrice
            ISNULL(g.costPrice, 0) as cost,
            ISNULL(g.goodsText, '') as description,
            ISNULL(g.goodsImg, '') as image_url,
            1 as is_active,
            GETDATE() as created_at,
            GETDATE() as updated_at,
            g.delflag
...
def migrate_orders(conn):
    ...
    try:
        ...
        SELECT 
            ...
            o.delState as order_state,  -- 使用delState而不是state
...
def main():
    """主函数"""
    logger.info("=" * 60)
    logger.info("🚀 最终正确版数据迁移脚本 - 从cyrg2025和cyrgweixin迁移到hotdog2030")
    logger.info("=" * 60)
