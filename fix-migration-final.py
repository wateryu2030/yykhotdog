...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fix_migration_final.log'),
...
def fix_migration_final(conn):
    ...
    try:
        ...
        SELECT 
            ...
            CASE 
                WHEN o.delflag = '系统删除' THEN 1
                WHEN o.delflag = 1 THEN 1
                WHEN o.delflag = 0 THEN 0
                ELSE 0
            END as delflag
        FROM Orders o
        WHERE (o.delflag = 0 OR o.delflag IS NULL OR o.delflag = '系统删除')
...
def main():
    ...
    # 最终修复数据迁移
    success = fix_migration_final(conn)
