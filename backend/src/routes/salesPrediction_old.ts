...
router.get('/performance/:storeId', async (req: Request, res: Response) => {
  try {
    ...
    // 计算增长率
    const salesGrowth = yesterday && yesterday.total_sales ? 
      ((today.total_sales - yesterday.total_sales) / yesterday.total_sales) : 0;
    const ordersGrowth = yesterday && yesterday.total_orders ? 
      ((today.total_orders - yesterday.total_orders) / yesterday.total_orders) : 0;
    const customersGrowth = yesterday && yesterday.customer_count ? 
      ((today.customer_count - yesterday.customer_count) / yesterday.customer_count) : 0;
...
router.get('/comparison/:storeId', async (req: Request, res: Response) => {
  try {
    ...
    const salesChange = previous.total_sales ? 
      ((current.total_sales - previous.total_sales) / previous.total_sales) : 0;
    const ordersChange = previous.total_orders ? 
      ((current.total_orders - previous.total_orders) / previous.total_orders) : 0;
    const customersChange = previous.total_customers ? 
      ((current.total_customers - previous.total_customers) / previous.total_customers) : 0;
    const avgOrderValueChange = previous.avg_order_value ? 
      ((current.avg_order_value - previous.avg_order_value) / previous.avg_order_value) : 0;
