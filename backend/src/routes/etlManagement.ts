...
router.get('/status', async (req: Request, res: Response) => {
  try {
    const etlPath = path.join(__dirname, '../../etl');
    ...
    const stepsWithStatus = ETL_STEPS.map(step => {
      const scriptPath = path.join(etlPath, 'steps', `${step.id.toString().padStart(2, '0')}_${step.name}.py`);
      return {
        ...step,
...
router.post('/execute-all', async (req: Request, res: Response) => {
  try {
    ...
    // 执行完整ETL流程
    const { stdout, stderr } = await execAsync(`python "${runScriptPath}"`, {
      cwd: etlPath,
