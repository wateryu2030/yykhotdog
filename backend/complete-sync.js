  ...
  try {
    ...
    const customersResult = await sourcePool.request().query(`
      SELECT 
        ...
        u.updated_at as last_order_date
      FROM [User] u
    ...
    for (let i = 0; i < customersResult.recordset.length; i += 1000) {
      ...
      const values = batch.map(customer => 
        `('${customer.customer_id}', '${customer.open_id || ''}', '${customer.vip_num || ''}', '${customer.phone || ''}', '${customer.nickname || ''}', '${customer.gender || ''}', '${customer.city || ''}', '${customer.district || ''}', '${customer.first_order_date || null}', '${customer.last_order_date || null}', 0, 0, 0, '新客户', GETDATE(), GETDATE())`
      ).join(',');
