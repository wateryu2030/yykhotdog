...
function getCustomerSegment(totalOrders, totalSpend) {
  if (totalOrders >= 10 && totalSpend >= 1000) return 'VIP客户';
  if (totalOrders >= 5 && totalSpend >= 500) return '高价值客户';
  if (totalOrders >= 3 && totalSpend >= 200) return '活跃客户';
  if (totalOrders >= 1 && totalSpend >= 100) return '普通客户';
  return '新客户';
}
...
async function finalFastSync() {
  let sourcePool, targetPool;
  
  try {
    console.log('🚀 开始最终高效数据同步...');
    ...
    const orderCountResult = await sourcePool.request().query(`
      SELECT 
        ...
        CONVERT(DECIMAL(10,2), round(SUM(total + vipAmount + vipAmountZengSong + cash) / CASE WHEN COUNT(*) = 0 THEN 1 ELSE COUNT(*) END, 2)) as avg_order_amount
    ...
    while (customerCount < userStats.total_users) {
      ...
      const customerBatchResult = await sourcePool.request().query(`
        SELECT 
          ...
          -- 城市名
          CASE WHEN CHARINDEX('-', o.shopName) > 0 THEN LEFT(o.shopName, CHARINDEX('-', o.shopName)-1) ELSE o.shopName END as city,
          -- 店铺名
          CASE WHEN CHARINDEX('-', o.shopName) > 0 THEN SUBSTRING(o.shopName, CHARINDEX('-', o.shopName)+1, LEN(o.shopName)) ELSE o.shopName END as store,
          NULL as vip_num,
          NULL as phone,
          NULL as nickname,
          NULL as gender,
          ...
          COUNT(*) as total_orders,
          SUM(o.total + o.vipAmount + o.vipAmountZengSong + o.cash) as total_spend,
          AVG(o.total + o.vipAmount + o.vipAmountZengSong + o.cash) as avg_order_amount
        FROM cyrg2025.dbo.Orders o
        WHERE o.delflag = 0 
          AND o.payState IN (1, 2, 3)
          AND o.openId IS NOT NULL 
          AND o.openId != ''
        GROUP BY o.openId, o.shopName
        ORDER BY COUNT(*) DESC
    ...
    // 4. 高效同步订单数据 - 使用较小的批次避免SQL Server限制
    console.log('\n📦 开始高效同步订单数据...');
    ...
    while (orderCount < stats.total_orders) {
      ...
      const orderBatchResult = await sourcePool.request().query(`
        SELECT 
          ...
          (o.total + o.vipAmount + o.vipAmountZengSong + o.cash) as total_amount,
...
// 运行最终高效同步
finalFastSync().catch(console.error); 